...
v5.0 — Contraseña propia encriptada, sin hardcodear nada
"""
...
import pandas as pd
import io, re, json, logging, unicodedata, hashlib, secrets, os
...
app = FastAPI(title="Cierre Mensual · Estudio Contable", version="5.0.0")
...
CONFIG_FILE   = BASE_DIR / "config.json"   # ← Contraseña encriptada acá
UPLOAD_DIR.mkdir(exist_ok=True)
OUTPUT_DIR.mkdir(exist_ok=True)


# ══════════════════════════════════════════════════════════════════════════════
# CONTRASEÑA — ENCRIPTADA CON SHA-256 + SALT
# El archivo config.json guarda solo el hash, nunca la contraseña en texto.
# Ni vos ni nadie puede leer la contraseña original.
# ══════════════════════════════════════════════════════════════════════════════
...
def hashear_password(password: str, salt: str) -> str:
    """Convierte la contraseña en un hash irreversible usando SHA-256 + salt."""
    return hashlib.sha256(f"{salt}{password}".encode()).hexdigest()

def sistema_configurado() -> bool:
    """Devuelve True si ya se creó una contraseña."""
    config = cargar_config()
    return "password_hash" in config and "salt" in config
...
def verificar_password(password: str) -> bool:
    ...
    if not config.get("password_hash") or not config.get("salt"):
        return False
    hash_ingresado = hashear_password(password, config["salt"])
    return hash_ingresado == config["password_hash"]

def verificar_admin(password: str) -> None:
    """Valida la contraseña. Lanza 403 si es incorrecta."""
...
# ══════════════════════════════════════════════════════════════════════════════
# FERIADOS — CÁLCULO AUTOMÁTICO PARA CUALQUIER AÑO
# ══════════════════════════════════════════════════════════════════════════════
...
def calcular_feriados(anio: int) -> set:
    f = set()
    for mes, dia in [(1,1),(3,24),(4,2),(5,1),(5,25),(6,20),(7,9),(12,8),(12,25)]:
        try: f.add(date(anio, mes, dia))
        except: pass
    pascua = calcular_pascua(anio)
    f.add(pascua - timedelta(days=48))
    f.add(pascua - timedelta(days=47))
    f.add(pascua - timedelta(days=2))
    f.add(date(anio, 6, 17))
    f.add(primer_lunes(anio, 8)  + timedelta(weeks=2))   # 3er lunes agosto
    f.add(primer_lunes(anio, 10) + timedelta(weeks=1))   # 2do lunes octubre
    f.add(primer_lunes(anio, 11) + timedelta(weeks=3))   # 4to lunes noviembre
    return f

def sumar_dias_habiles(fecha: date, dias: int) -> date:
    actual, n = fecha, 0
    feriados  = calcular_feriados(fecha.year)
    while n < dias:
        actual += timedelta(days=1)
        if actual.year != fecha.year:
            feriados = calcular_feriados(actual.year)
...
def conciliar(df_afip: pd.DataFrame, df_cli: pd.DataFrame) -> dict:
    ...
    for _,row in dm.iterrows():
        base = {"cuit":str(row.get("cuit_afip",row.get("cuit_cli",row.get("cuit","")))),
        ...
        if row["_merge"]=="left_only":
            ...
            ta=float(row.get("total_afip",0) or 0); tc=float(row.get("total_cli",0) or 0); df=abs(ta-tc)
...
@app.get("/")
def root():
    return {"mensaje":"Cierre Mensual v5.0 ✅","configurado":sistema_configurado()}

@app.get("/health")
def health():
    return {"status":"ok","configurado":sistema_configurado(),"timestamp":datetime.now().isoformat()}

# ── Estado del sistema ─────────────────────────────────────────────────────────
@app.get("/api/estado")
def estado():
    """
    El frontend llama a este endpoint al arrancar para saber
    si tiene que mostrar la pantalla de 'Crear contraseña' o la de login.
    """
    return {"configurado": sistema_configurado()}
...
def setup(req: SetupRequest):
    """
    Solo funciona si aún no hay contraseña configurada.
    Crea el hash + salt y lo guarda en config.json.
    """
    if sistema_configurado():
        raise HTTPException(400, "El sistema ya tiene una contraseña configurada")

    if len(req.password) < 6:
        raise HTTPException(400, "La contraseña debe tener al menos 6 caracteres")

    if req.password != req.confirmar:
        raise HTTPException(400, "Las contraseñas no coinciden")

    # Generar salt aleatorio único para este sistema
    salt = secrets.token_hex(32)
    password_hash = hashear_password(req.password, salt)
    ...
    logger.info("✅ Contraseña configurada correctamente")
    return {"ok": True, "mensaje": "¡Contraseña creada! Ya podés ingresar al sistema."}
...
def login(password: str):
    if not sistema_configurado():
        raise HTTPException(400, "El sistema no tiene contraseña configurada todavía")
...
def cambiar_password(req: CambiarPasswordRequest):
    ...
    if not verificar_password(req.password_actual):
    ...
    if req.password_nueva != req.confirmar:
        raise HTTPException(400, "Las contraseñas nuevas no coinciden")

    config = cargar_config()
    config["password_hash"] = hashear_password(req.password_nueva, config["salt"])
    config["actualizado"]   = datetime.now().isoformat()
...
def agregar(cliente: ClienteNuevo, password: str):
    ...
    c=cargar_clientes(); cid=nombre_a_id(cliente.nombre)
    if cid in c:
        i=2
...
def actualizar(cid:str, datos:ClienteActualizado, password:str):
    ...
    if datos.cuit and not es_cuit_valido(datos.cuit): raise HTTPException(400,f"CUIT inválido: {datos.cuit}")
...
def eliminar(cid:str, password:str):
    ...
    c=cargar_clientes()
    if cid not in c: raise HTTPException(404,"Cliente no encontrado")
    nombre=c[cid]["nombre"]; del c[cid]; guardar_clientes(c)
...
async def conciliar_endpoint(
    afip_ventas:UploadFile=File(...), afip_compras:UploadFile=File(...),
    excel_cliente:UploadFile=File(...), periodo:str="2026-04", password:str=""):
    verificar_admin(password)
    try:
        df_v=leer_csv_afip(await afip_ventas.read()); df_c=leer_csv_afip(await afip_compras.read())
        df_a=pd.concat([df_v,df_c],ignore_index=True)
        df_cl=leer_excel_cliente(await excel_cliente.read())
        res=conciliar(df_a,df_cl); res["periodo"]=periodo
        ts=datetime.now().strftime("%Y%m%d_%H%M")
        with open(OUTPUT_DIR/f"conciliacion_{periodo}_{ts}.json","w",encoding="utf-8") as f:
            json.dump(res,f,ensure_ascii=False,indent=2,default=str)
        return res
    except Exception as e:
        logger.error(f"❌ {e}"); raise HTTPException(500,str(e))


# ── Exportar reporte Excel ─────────────────────────────────────────────────────
@app.post("/api/exportar/{cliente_id}")
async def exportar_reporte(
    cliente_id:str, afip_ventas:UploadFile=File(...), afip_compras:UploadFile=File(...),
    excel_cliente:UploadFile=File(...), periodo:str="2026-04", password:str=""):
    verificar_admin(password)
    try:
        clientes=cargar_clientes(); cliente=clientes.get(cliente_id,{"nombre":cliente_id})
        df_v=leer_csv_afip(await afip_ventas.read()); df_c=leer_csv_afip(await afip_compras.read())
        df_a=pd.concat([df_v,df_c],ignore_index=True)
        df_cl=leer_excel_cliente(await excel_cliente.read())
        ...
        stats_df=pd.DataFrame([{"Cliente":cliente["nombre"],"Período":periodo,
            "Total AFIP":res["stats"]["total_afip"],"Total Cliente":res["stats"]["total_cliente"],
            "Discrepancias":res["stats"]["discrepancias"],"Críticas":res["stats"]["criticas"],
            "% OK":f"{res['stats']['porcentaje_ok']}%","Generado":datetime.now().strftime("%d/%m/%Y %H:%M")}])
        with pd.ExcelWriter(buf,engine="openpyxl") as writer:
            stats_df.to_excel(writer,sheet_name="Resumen",index=False)
            if not disc.empty:
                cols=["emoji","tipo_alerta","cuit","descripcion","comprobante","total_afip","total_cliente","diferencia","prioridad"]
                disc[[c for c in cols if c in disc.columns]].to_excel(writer,sheet_name="Discrepancias",index=False)
...
def vencimientos(periodo:str, password:str):
    verificar_admin(password)
    try: anio,mes=int(periodo[:4]),int(periodo[5:7])
    except: raise HTTPException(400,"Formato inválido. Usar YYYY-MM")
    hoy=date.today(); c=cargar_clientes(); res=[]
    for cid,cl in c.items():
        cuit_l=re.sub(r"\D","",cl["cuit"]); venc=calcular_vencimiento_iva(cuit_l,anio,mes)
        dias=(venc-hoy).days
        res.append({"cliente_id":cid,"nombre":cl["nombre"],"cuit":cl["cuit"],
            "concepto":f"IVA {mes:02d}/{anio}","fecha_vencimiento":venc.strftime("%d/%m/%Y"),
            "dias_restantes":dias,"urgencia":"rojo" if dias<=2 else "amarillo" if dias<=7 else "verde",
            "vencido":dias<0})
